...
function ViewXComponent(props) {
    ...
    return (_jsx(Fragment, { children: (type === "applicationRoot")
            ? jsonxChildren
            : el ? ReactDOM.createPortal(jsonxChildren, el) : null }, "viewx"));
}
export default function getMainComponent(options) {
    if (!options)
    ...
    function Main(appProps) {
        ...
        /* eslint-enable */
        return (_jsx(Fragment, { children: config.layers.map(layer => {
                return (_jsx(ViewXComponent, { layer: layer, views: views, viewdata: viewdata, ctx: ctx, layerStates: layerStates, settings: settings }, void 0));
            }) }, "viewx"));
    }
    return Main;
}
